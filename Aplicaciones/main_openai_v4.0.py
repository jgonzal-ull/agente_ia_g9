...
v4.0: sincronización de Estado en el store + bot de Telegram.
"""
...
def chatear_simple(indice: VectorStoreIndex, llm: Any, pregunta: str) -> str:
    ...
    logger.info("Iniciando conversación con el agente (usando la API de OpenAI):")
    responses: List[str] = []
    ...
    try:
        ...
        responses.append("\nError durante la conversación. ¿Hay conexión con la API de OpenAI?")
        responses.append(f"¿Es válida la clave OPENAI_API_KEY y el modelo '{getattr(llm, 'model', '?')}'?")
...
def main() -> None:
    ...
    global index
    print("=== Agente RAG con OpenAI v4.0 (bot Telegram) ===\n")
